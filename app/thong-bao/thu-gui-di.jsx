...
export default function SalaryNotificationList({ notifications = [], onSelect }) {
  // Hàm để tạo tiêu đề thông báo dựa trên loại cảnh báo
  const getNotificationTitle = (item) => {
    if (!item) return "Thông báo mới"
  ...
  const getPreviewContent = (item) => {
    if (item.alert_type === "leave_exceed") {
            ...
            return (
              ...
              >
                ...
                <div className="text-sm text-gray-700 min-w-[220px] mr-6">
                  <div className="font-medium">Phòng Nhân sự</div>
                  <div className="text-gray-500">hr@company.com</div>
                </div>

                {/* Notification content (title + preview) */}
                <div className="flex-1 flex flex-col justify-center mr-6">
                  <div className="text-sm font-medium text-gray-900 mb-1">{getNotificationTitle(item)}</div>
