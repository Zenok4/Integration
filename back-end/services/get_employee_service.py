...
def get_employee(employee_ids=None):
    ...
    if not employee_ids:
        return {"message": "No employee IDs provided", "employees": []}

    # 🔹 Lấy danh sách nhân viên từ MySQL
    mysql_conn = get_mysql_connection()
    mysql_cursor = mysql_conn.cursor(dictionary=True)
    format_strings = ','.join(['%s'] * len(employee_ids))
    mysql_cursor.execute(f"SELECT * FROM employees WHERE employee_id IN ({format_strings})", tuple(employee_ids))
    mysql_employees = {emp["employee_id"]: emp for emp in mysql_cursor.fetchall()}
    mysql_conn.close()
